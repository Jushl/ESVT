...
from models.ESVT.decoder.denoising import get_contrastive_denoising_training_group
from models.ESVT.utils import get_activation, inverse_sigmoid, bias_init_with_prob, deformable_attention_core_func_v2
...
if __name__ == '__main__':
    m = RTDETRTransformerv2(name='L')

    data1 = torch.randn(2, 256, 64, 64)
    data2 = torch.randn(2, 256, 32, 32)
    data3 = torch.randn(2, 256, 16, 16)
    data = [data1, data2, data3]

    target1 = {
        'boxes': torch.tensor([[0.3080, 0.6520, 0.6161, 0.6819]]),
        'labels': torch.tensor([23]),
    }

    target2 = {
        'boxes': torch.tensor([
            [0.4448, 0.3240, 0.0169, 0.0221],
            [0.7154, 0.3223, 0.0232, 0.0410],
            [0.5445, 0.3187, 0.0318, 0.0234]
                               ]),
        'labels': torch.tensor([2, 2, 2]),
    }
    targets = [target1, target2]

    output = m(data, targets=targets)
    print(output)
